...
local function MakeInlineDesc(recipe)
	...
	for k,v in pairs(recipe) do
		...
		local has = LocalPlayer():HaveItem("entity", k, v)
		if not has then
			local tbl = Inventory.GetItem("entity", k)
			str = str.. (tbl and tbl.name or "ERROR_ITEM_NOT_EXIST") ..": <color=230,50,50>x"..v.."</color>  "
		else
			str = str..R_CraftSystem.Details[k].name..": <color=50,230,50>x"..v.."</color>  "
		end
...
local function MakeInlineDescRazbor(item)
	...
	for k,v in pairs(R_CraftSystem.Disassembly[item.class].player_get) do
		parscounter = parscounter + 1
		str = str..R_CraftSystem.Details[k].name..": <color=50,200,50>x"..v.."</color>  "
		if parscounter >= 4 then
			str = str .. "\n"
			parscounter = 0
		end
...
local function OpenCategory(base, cat)
	...
	base.craftinfo.resulttext = "Шанс: 100%"
	...
	base.craftinfo.Paint = function(slf, w, h)
		...
		draw.ShadowSimpleText( "Создать " .. (weapons.GetStored(base.craftinfo.craftname) and weapons.GetStored(base.craftinfo.craftname).PrintName or "Неизвестно"), "rp.ui.32", w*0.45, 10, Color(255,255, 255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP )
		...
		if not base.craftinfo.weppreview then
			...
			base.craftinfo.weppreview:SetModel( weapons.GetStored(base.craftinfo.craftname).WorldModel )
	...
	base.craftinfo.btn.DoClick = function()
		if (CurTime() > base.craftinfo.btn.craftcd) then
			if CanCraftRecipe(base.craftinfo.crafttbl.recipe) then
	...
	local recipes = R_CraftSystem.Recipes
...
function OpenCraftMenu(category) 
