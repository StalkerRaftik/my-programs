  ...
  for z, types in pairs(rp.Kits.Config) do
    ...
    for index, kit in pairs(types) do
        ...
        local ButtonFunction = function()
          if LocalPlayer():GetNWInt("AllowedToUse"..z.."KitsTime") >= os.time() or ( kit.Upgrade and not LocalPlayer():AllowedToUseKit(kit) ) and not ValidPanel(ErrorMsg) then
        ...
        KitDescription:SetSize(DropButton:GetWide()/1.5,DropButton:GetTall()) 
        KitDescription:SetPos(5, 0)
