...
timer.Create( "IfIInTerritoryAndNearFlag", 1, 0, function()
...
hook.Add("PostDrawHUD", "ConquestUI", function()
	...
	if headerh ~= 0 or flag ~= nil then
		draw.ShadowSimpleText( 'За удержание этой территории в склад будут поступать:', "font_base_24", ScrW()/2, ScrH()*0.08, Color(230,230,230,255*headerh), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
		draw.ShadowSimpleText( text, "font_base_24", ScrW()/2, ScrH()*0.11, Color(230,230,230,255*headerh), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
	end
	...
	draw.ShadowSimpleText( tname, "font_base", 5, ScrH()*0.01, Color(230,230,230,255*conhud), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
